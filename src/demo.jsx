...
function TallySection() {
  ...
  return <section id="tally" className="bg-stone text-ink"><SectionHeader number="II" eyebrow="THE TALLY OF DEEDS" title="Let no weight once borne fall into darkness." sequence="WHAT WAS → WHAT IS" stone /><div className="grid bg-stone text-ink lg:grid-cols-[0.72fr_1.28fr]"><div className="border-b border-ink px-6 py-16 lg:border-b-0 lg:border-r lg:px-[clamp(26px,5vw,78px)] lg:py-24"><p className={eyebrow}>THE BRONZE-FAST MEMORY</p><p className="max-w-xl text-[17px] leading-relaxed text-ink-soft md:text-xl">When the iron-laden bar returns to the hands, the former weight returns beside it. The lighter preparation stands apart, but the true work is cut into the tally.</p><blockquote className={`${display} mt-10 border-t-[3px] border-double border-bronze pt-5 text-2xl leading-tight md:text-4xl`}>What was done is not lost; it stands beside what must now be done.</blockquote></div><div className="grid place-items-center p-6 md:p-12"><div className="w-full max-w-3xl border-2 border-ink bg-paper-raised text-ink outline outline-1 outline-offset-8 outline-bronze"><header className={`${display} flex min-h-16 items-center justify-between border-b-[3px] border-double border-ink px-4 text-xl`}><span>BARBELL BACK SQUAT</span><span className="text-sm font-bold text-red">3 × 6–10</span></header><div className="grid grid-cols-[44px_1.2fr_.8fr_.8fr_54px] border-b border-ink text-center font-display text-[7px] font-bold tracking-widest">{["SET", "BEFORE", "KG", "REPS", "SEALED"].map((label) => <span key={label} className="grid min-h-10 place-items-center border-r border-ink last:border-r-0">{label}</span>)}</div>{rows.map(([set, before, weight, reps, sealed]) => <div key={set} className={`grid min-h-16 grid-cols-[44px_1.2fr_.8fr_.8fr_54px] border-b border-ink ${set === "W" ? "bg-stone/50" : ""}`}>{[set, before, weight, reps, sealed ? "✓" : ""].map((value, index) => <span key={index} className={`grid place-items-center border-r border-ink last:border-r-0 ${index === 0 ? "font-display text-lg text-red" : ""} ${index === 2 || index === 3 ? "font-display text-xl" : ""}`}>{value}</span>)}</div>)}<footer className={`${display} flex min-h-12 items-center justify-between gap-3 px-4 text-[7px] font-bold tracking-wider`}><span>W / MAKING READY</span><span>WEIGHT APPOINTED / 62.5 KG × 8</span></footer></div></div></div></section>;
}
